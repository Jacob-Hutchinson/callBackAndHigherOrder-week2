...
const evenNum = mixedNumbers.filter((element)=>{
  return element % 2 === 0
})
...
*/
 ...
 const totalPopulation  = populations.reduce((acc, element)=>{
  return acc + element
 })
...
const totalPrice = orders.map((element)=> {
...
const bobTotal = purchases.filter((element)=>element.owner === "Bob").reduce((acc,curr)=> acc + curr.price, 0)
